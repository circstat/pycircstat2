...
class CLRegression:
    ...
    - **"gam"** (default): ``hea.gam`` with a :class:`CircularLL` general
      ...
      — a ``*lss`` alias (``vmlss`` the default, ``wclss``, … ``kjlss``), a
      bare regression-ready distribution (auto-wrapped), or a constructed
      ``CircularLL(dist, links=...)`` — plus penalized smooths
      (``s()``/``te()``), REML, and **different covariates per LP**.
      ``CLRegression(["θ ~ s(x)", "~ s(z)"], data)`` smooths μ(x)
      and log κ(z) jointly. Use ``family=pnlss`` when μ(x) must
    ...
    **Formula grammar (both backends):** a string ``"θ ~ rhs"`` is sugar for
    ...
    def _init_gam(self, formula, data, family, knots, method, gam_kwargs):
        ...
        predictor, ``hea.gam`` + a :class:`CircularLL` family (default
        ``vmlss``), REML by default. A single formula gets a constant second
    ...
    def shape_inference(self, data=None, *, level=0.95, cov="Vp"):
        ...
        if not isinstance(self.family, KatoJonesLL):
            raise ValueError(
                ...
                "parameters; fit with family=kjlss (KatoJonesLL — or bare "
                "family=katojones)."
            )
